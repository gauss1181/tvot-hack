var preview = 
{
	...
	scaleFrame : function(externalPreview, resizing, fullScreen) {
		...
		var previewWindow = preview.parentWindow || window,
			previewDoc = previewWindow.document,
			iFrame = $("#previewPage", previewDoc),
			iFrameWindow = window;
		...
		// we are previewing outside the iframe or not previewing
		if(!externalPreview && !iFrame[0]) {
		...
		var scale,
			previewShare = iFrame[0].ownerDocument.querySelector("#previewShare"),
			previewShareWidth = previewShare && !fullScreen && !previewShare.classList.contains('closed')
				? previewShare.clientWidth: 0,
			previewMenuWidth = fullScreen? 0: previewMenu.width,
			slider = previewDoc.getElementById('previewZoomSlider');

		// use value of scale slider
		if (!fullScreen && slider && iFrameWindow.preview && iFrameWindow.preview.useZoomSlider) {
			scale = slider.value;
		}
		else {
			var frameWidth = iFrame.outerWidth(),
				frameHeight = iFrame.outerHeight(),
				maxDimension = Math.max(frameWidth, frameHeight),

				iFrameSize = (fullScreen
					? { width: frameWidth  , height: frameHeight  }
					: { width: maxDimension, height: maxDimension }),

				windowSize = (fullScreen
					? { width: previewWindow.innerWidth, height: previewWindow.innerHeight }
					: preview.availableRect),

				fitSize = utils.getFitSize({
					object: iFrameSize,
					frame: windowSize,
					expand: fullScreen
				});

			scale = fitSize.width / iFrameSize.width;

		}

		// If you're not using the slider's value for zoom
		// set the slider's value to the calculated zoom
		if (!preview.useZoomSlider && slider && typeof scale === 'number') {
			var $slider = previewWindow.$(slider);
		...
		// The slider's value will be the fullScreen zoom so this will make the zoom persist
		if (fullScreen) {
			if (previewWindow === window) {
				iFrame[0].contentWindow.preview.useZoomSlider = true;
			}
			else {
				preview.useZoomSlider = true;
			}
		}
		...
		preview.centerFrame(previewWindow);
	...
	updateExportUi: function(result) {
		var links = {
			Pages: $('#previewShareContainer .shareLinkExportImg').add('#projectSettingsShareTop .shareLinkExportImg'),
			Project: $('#previewShareContainer .shareLinkExportImgFull').add('#projectSettingsShareTop .shareLinkExportImgFull')
		};
		var $msgBox;
...
//			}
	...
	open: function (e, p) // p = page object = optional.
	{
		...
		preview.hideNotes();
		if (document.getElementById('previewShowNotes').checked) {
		...
		$("#previewPage")
			...
			.attr('src', 'project/' + projectId );

		$("#previewHolder").fadeIn(200).addClass("open");

		preview.displaySharePanel();
		preview.setAvailableRect(preview.calcAvailableRect());
		preview.centerFrame();
	...
	afterPreviewLoad: function(projectData) {
		...
		// get the space available for the iFrame to be scaled & centered in
		try {
			preview.parentWindow.preview.setAvailableRect();
		}
		catch (error) {
			preview._crossDomainCall(window.parent, 'setAvailableRect');
		}
	...
	load : function(previewId, callback)
	{
		...
		try {
			preview.parentDoc = window.frameElement.ownerDocument;
			preview.parentWindow = preview.parentDoc.defaultView;
		}
		catch (e) {
			preview.parentWindow = null;
		}
	...
	draw : function(data)
	{
		...
		for (var i = 0; i < proj.pages.length; i++) {
			var notesArray = storage.get([proj.pages[i]]);

			preview.pageNotes[proj.pages[i]] = notesArray && notesArray[3] || '';
		}
		...
		if (preview.parentWindow) {
			if (!window.g_embedded) {
				try {
					...
					if (parent.size() > 0) {
						parent.css({width: proj.pageWH[0], height: proj.pageWH[1]});  //
			...
			preview._crossDomainCall(window.parent, 'setFrameSize', proj.pageWH);
		...
		{
			var frameStartPage = preview.parentWindow? preview.parentWindow.preview.startPage: null,
				home = frameStartPage ||
					(proj.homepage ? proj.homepage : proj.pages[0]); // the project's homepage

			preview.homePage = home;
			...
			{
				preview.scaleViewport();
				// preview.applyPageTransforms($('#home'));
	...
	drawPage : function(pageId, proj)
	{
		...
		if (window.g_print) {
			// pageElem is the #print_x_... element that contains the page element

			if (p.width === proj.pageWH[0] && p.height === proj.pageWH[1]) {
				pageElem.addClass('project-size');
			}
		}
	},
	...
	changePage : function(page, args, navigateBack, forceNoTransition)
	{
		...
		preview.orient(null, $(page), $('.ui-page.ui-page-active'));
		...
		// window.Android is inserted by an Android WebView (for the Android app)
		// Android.updateOrientation is actually a java method, this is just a trigger for it
		if (window.Android) {
			Android.updateOrientation();
		}
	...
	_orientMobile : function(pageElem, prevPageElem, pageData)
	{
		...
		var translateSide;
		if ((pageData.portrait && pageData.projectOriented)
			|| (!pageData.portrait && !pageData.projectOriented)) {
			translateSide = 'width';
		}
		else {
			translateSide = 'height';
		}

		preview.pageTranslation = preview.pageRotation
			? (preview.pageRotation > 0
				? [0, -pageData[translateSide]]
				: [-pageData[translateSide], 0])
			: [0, 0];
	...
	orientPrevPage : function (pageElem, prevPageElem, pageData) {
		if (!prevPageElem) { return; }

		var transform,
			portraitProject = (pageData.portrait && pageData.projectOriented )|| (!pageData.portrait && !pageData.projectOriented)
			translateSide = portraitProject? 'width': 'height';

		if (pageData.portrait) {
			transform = 'rotate(90deg) translate(0,-' + pageData[translateSide] + 'px)';
		}
		else {
			transform = 'rotate(-90deg) translate(-' + pageData[translateSide] + 'px, 0)';
		}
	...
	// orientPage : function (pageElem, prevPageElem, pageData) {
	...
	_orientParent : function (pageElem, prevPageElem, pageData) {
		// the iframe
		var parent = $('#previewPage', preview.parentWindow? frameElement.ownerDocument: document);

		pageData = pageData || preview._orientationData(pageElem);

		// if page orientation is different from the current frame orientation
		if (pageData.crossOriented) {

			// If _orientParent instruction will be posted to the parent window,
			// delay the rotation of the page so that it happens at around the
			// same time the parent window recieves the posted message.
			// This prevents flickering in the page
			if (!preview.parentWindow && window.g_preview) {
				setTimeout(function () {
					preview.orientPrevPage(pageElem, prevPageElem, pageData);
				}, 0);
			}
			else {
				preview.orientPrevPage(pageElem, prevPageElem, pageData);
			...
			if (parent && parent.length) {
				...
				// set up frame position and dimensions for new page
				// rotate the frame and the page so that the current page looks like it has no transformation
				// run the frame rotation transition
				// *then* run the page change transition;
				// clear the transformations in the end (in finishOrient())
				...
				if (pageData.projectOriented) {
					newSize.width = pageData.width;
					newSize.height = pageData.height;
				} else {
					newSize.width = pageData.height;
					newSize.height = pageData.width;

					// preview.orientPage(pageElem, prevPageElem, pageData);
				}

				$('.widgetHolder', pageElem).css({
		            '-webkit-transform': '',
		            '-moz-transform': '',
		            'transform': '',
				...
				preview.centerFrame(window.parent || window);

				if (pageData.portrait) {
					iFrameParent.classList.remove('to-landscape');
					iFrameParent.classList.add('to-portrait');
				}
				else {
					iFrameParent.classList.remove('to-portrait');
					iFrameParent.classList.add('to-landscape');
				}
			}
			else if (window.parent !== window) {
	...
	clearPageOrientation : function () {
		preview.pageRotation = 0;
		preview.pageTranslation = [0, 0];
	},

	_finishOrient : function(pageElem, prevPageElem, pageData) {
		var parent = preview.parentWindow && window.parent && $('#previewPage', window.parent.document); // the iframe

		pageData = pageData || preview._orientationData(pageElem);

		if (preview.parentWindow && pageElem) {
			preview.multiOrientations = pageData.multiOriented;

			preview.updateNotes(pageElem[0].id);
			preview.positionNotes();
			preview.showNotes();
		}

		if (prevPageElem) {
			preview.clearPageOrientation();
			prevPageElem.find('.widgetHolder').css({
				transform: '',
				webkitTransform: '',
				mosTransform: ''
			});
	...
	runDraw : function(dat)
	{
		...
		prjObj = prjObj || dat[window.g_preview];

		preview.loaded = true;
		preview.navigationStackBack = [];
		preview.navigationStackForward = [];
		preview.navigationQueueNext = [];
		preview.navigationQueueFlow = [];
		...
		if (window.g_print) {
			var printMargin = 20,
				width = prjObj.pageWH[0]
				height = prjObj.pageWH[1],
				// page orientation and default .printPageWrapper size
				printStyle = '@page { size: ' + prjObj.orientation + '; margin: ' + printMargin + 'px; }\n' +
							'.printPageWrapper { width: ' + width + 'px; height: ' + height + ' }\n';

			// Shrink to fit within the print paper (pages equal to device size)
			for (var scale = 1; scale > 0; scale -= 0.1) {
				scale = scale.toFixed(1);

				var scaledW = Math.floor(width * scale),
					scaledH = Math.floor(height * scale);

				printStyle += '@media print and (max-width: ' + (scaledW + printMargin) + 'px) {\n' +
								'    .project-size .page { -webkit-transform: scale(' + scale + '); transform: scale(' + scale + '); }\n' +
								'    .printPageWrapper.project-size { width: ' + scaledW + 'px; height: ' + scaledH + 'px}\n' +
								'}\n';
			}

			for (var i = 0; i < prjObj.pages.length; i++) {
				var pageId = prjObj.pages[i],
					page = dat[pageId];

				// do nothing if page size is the project size
				if (page.width === width && page.height === height) {
					continue;
				}

				printStyle += '#print_' + pageId + ' { width: ' + page.width + 'px; height: ' + page.height + 'px; }\n';

				// Shrink individual page to fit within the print paper
				for (var scale = 1; scale > 0; scale = scale -= 0.1) {
					scale = scale.toFixed(1);

				var scaledW = Math.floor(page.width * scale),
					scaledH = Math.floor(page.height * scale);

					printStyle += '@media print and (max-width: ' + (scaledW + printMargin) + 'px) {\n' +
								'    #' + pageId + ' { -webkit-transform: scale(' + scale + '); transform: scale(' + scale + '); }\n' +
								'    #print_' + pageId +' { width: ' + scaledW + 'px; height: ' + scaledH + 'px; }\n' +
								'}\n'
				}
			}
	...
	restart : function()
	{
		preview.changePage("#" + preview.homePage, null, false, true);
	},
	...
	getNotesElement : function (targetWindow) {
		targetWindow = targetWindow || window;

		var notesElement = targetWindow.document.getElementById('inPreviewNotes');
		...
		try {
			if (targetWindow.frameElement) {
				return preview.getNotesElement(targetWindow.frameElement.ownerDocument.defaultView);
			}
		}
		catch (error) {}
	...
	positionNotes : function () {
		var notes = preview.getNotesElement(),
			width = notes && notes.clientWidth || 0;

		if (notes) {
			var frame = document.getElementById('previewPage') || window.frameElement,
				frameRect = frame.getClientRects()[0];
			...
			notes.style.left = (x - width + 1) + 'px';
			notes.style.top = Math.max(y, 0) + 'px';
		}
	},

	updateNotes : function (pageId) {
		var notesElement = preview.getNotesElement();

		if (!notesElement) { return; }

		var notesContainer = preview.getNotesElement().querySelector('#previewNotesContent'),
			notesText = page.get(pageId, 'notes');

		notesContainer.textContent = notesText && notesText[3] || 'This page has no notes';
	},

	showNotes : function () {
		notesElement = preview.getNotesElement();

		notesElement && notesElement.classList.remove('hide');
	},

	hideNotes : function () {
		notesElement = preview.getNotesElement();

		notesElement && notesElement.classList.add('hide');
	},

	toggleShare : function () {
		var shareMenu = document.getElementById('previewShare');
		
		shareMenu.classList.toggle('closed');
	...
	centerFrame : function (targetWindow) {
		targetWindow = targetWindow || preview.parentWindow || window;

		var doc = targetWindow.document,
			iFrame = doc.getElementById('previewPage');

		if (!iFrame) {
			preview._crossDomainCall(window.parent, 'centerFrame');
			return;
		}

		var iFrameParent = iFrame.parentNode,
			parentStyle = iFrameParent.style,
			areaRect = preview.availableRect,

		width = $(iFrame).outerWidth(),
		height = $(iFrame).outerHeight(),

		left = areaRect.width / 2 + areaRect.left,
		top = areaRect.height / 2 + areaRect.top;

		parentStyle.marginLeft = -(width / 2) + 'px';
		parentStyle.marginTop  = -(height / 2) + 'px';
		...
		// give time for transitions to end
		setTimeout(preview.positionNotes, 400);
	},

	calcAvailableRect : function (targetWindow) {
		targetWindow = targetWindow || window;

		var doc = targetWindow.document,

			iFrame       = doc.getElementById('previewPage'),
			previewShare = doc.getElementById('previewShare'),
			shareRect    = previewShare && !previewShare.classList.contains('closed') && previewShare.getClientRects()[0],
			shareWidth   = shareRect? shareRect.width: 0;

		return {
			width : targetWindow.innerWidth - (targetWindow.previewMenu? targetWindow.previewMenu.width: 0) - shareWidth,
	...
	setAvailableRect : function (rect) {
		rect = rect || preview.calcAvailableRect();

		preview.availableRect = rect;

		if (preview.canAccessFrame() && $('#previewPage')[0].contentWindow.preview) {
			$('#previewPage')[0].contentWindow.preview.availableRect = rect;
		}
		else if ($('#previewPage').length) {
			preview._crossDomainCall($('#previewPage')[0].contentWindow, 'setAvailableRect', rect);
		}
	},

	canAccessFrame : function () {
		try {
			return !!(document.getElementById('previewPage').contentWindow.document);
		}
		catch (e) {
			return false;
		}
...
var page = 
{
	...
	{
		var activePage = project.getActivePageId();
	...
	click : function(e)
	{
		...
		{
			if ($(e.target).closest(".screen").attr("id") == project.getActivePageId()) return; // ignore the current page.
...
//		}
	...
	*/
	...
	selectStop : function(e) {
		...
		var selected = document.querySelectorAll('#canvasPages .ui-selected');

		widget.resetSelection();
		widget.selectMulti = true;

		fluid.util._.forEach(selected, function(widgetElement) {
			var id = widgetElement.dataset.lockto;

			// final argument true to not place/hide the context menu
			widget.select("pass", $("#"+id), true);
		});
	...
	drawRasterised: function(id, callback, hideHotspots) {
		...
		this.renderer.renderPage(pageObj, hideHotspots);
    ...
    drawDetailed: function(id, mode, options) {
        ...
        if(options && options.restoreSelection) {
            $('#'+options.restoreSelection).click();
        }
    },
...
var grid = 
{
	...
	{
		var currentPageID = project.getActivePageId();
...
var widget = 
{
	...
	get : function(id)
	{
		...
		{
			data = widgets.summary && widgets.summary[id] && $.extend(true, {id : id}, widgets.summary[id]);
		}
		if(data == undefined)
		{		
			data = storage.get(id);
    ...
    getSubTextElements: function(widgElement) {
        var response = [];

		var widgId = widgElement.id,
			allSubs = widget.getDependents(widgId),
			wdgObj = widget.get(widgId);

        fluid.util._.forEach(allSubs, function(el){
            response = response.concat(widget.getSubTextElements(el));
        });

        if( wdgObj.tc ) {
            response.push(widgElement);
        }
    ...
    getRoots : function(sel) // TODO: we can store this data locally if this select is slow?
	{
		...
		fluid.util._.forEach(sel, function(widgetElement) {
			if (widgetElement.classList.contains('rootWidget')) {
				all.push(widgetElement);
			}
			else {
				var lockTo = widgetElement.dataset.lockto,
					siblings = widgetElement.parentNode.children;

				for (var i = 0, len = siblings.length; i < len; i++) {
					if (siblings[i] !== widgetElement && $(siblings[i]).is('.rootWidget[data-lockto=' + lockTo + ']')) {
						all.push(siblings[i]);
					}
				}

				// $(this).siblings(".rootWidget").filter('[data-lockto="' + lockTo + '"]')
			}
		});
		
		return $(fluid.util._.uniq($(all)));
	},
	
	getLockTos : function(sel) {
		var roots = widget.getRoots(sel);

		var allRoots = Array.prototype.reduce.call(roots, function(all, element) {
			fluid.util._.forEach(
				document.querySelectorAll('[data-lockto="' + element.id + '"]'),
				function (hasSameLockTo) {
					if (all.indexOf(hasSameLockTo) === -1) {
						all.push(hasSameLockTo);
					}
				});
	...
	getDependents : function (widgId, lockTos, useCache) {	
		...
		// speedup: build a tree rather than doing this every iteration.
		var allDependants = fluid.util._.reduce(lockTos, function (all, element) {
			var subWidgObj = widget.get(element.id);
	...
	autoWidth : function(data,mode) // Update Widgets with auto with tag 
	{
		...
		var pagew = mode != 'library' ? project.getActivePage().find('.widgetHolder').width() : 320;
	...
	widget.hover = function(e)
	{
        ...
        if(e.type === 'mouseover') widget.hoverIn.call(this, e);
        else widget.hoverOut.call(this, e);
	};

    widget.hoverOut = function(e) {
        clearTimeout(widget.hoverTimeout);
        if(widget.$linksToFade) widget.$linksToFade.fadeIn(100);
        $('.link').removeClass('front');
    };

    widget.hoverIn = function(e) {
        //widget.hoverOut();
        widget.hoverTimeout = setTimeout(widget.fadeUnrelatedLinks.bind(this, e), 500);
    };

    widget.fadeUnrelatedLinks = function(e) {
        var originId = $(this).attr("id");
        var linkId = link.from(originId);
        if(!linkId) return;
        var $linksInDom = $('.link');

        widget.$linksToFade = $linksInDom.filter(function() {
            if(this.id === linkId) return false;
            else return true;
        });

        widget.$linksToFade.fadeOut(100);

        $('#'+linkId).addClass('front');
    };
	...
	widget.click = function(e)
	{
		...
		if (!$('#canvas').data('zooming'))
		{
			...
			var activeScreenId = project.getActivePageId();
	...
	widget.addDefault = function(e)
	{
		...
		if(active.length == 1)
		{
			...
			if(widgets.summary[id].dp)
			{
				dp.top = widgets.summary[id].dp[0] ;
				dp.left = widgets.summary[id].dp[1] ;
			}
			var wh = active.find(".widgetHolder");

			widgetId = widget.add(e, {top: wh.offset().top + dp.top -5, left: wh.offset().left + dp.left -5}, id, wh); // TODO: -5 is a horrible fix to the drop snap problem.
            tracker.record("addWidget", 1, id, e);
		}
	...
	widget.getImageData = function(id, tryFromCloud) {
		var uploadObj = storage.get(id);
		var response = id || false;
		if(response && response.substr(0, 5) != 'data:')
		{
			if(tryFromCloud && (!uploadObj || !uploadObj.storage || uploadObj.storage == 'cloud') ) response = imgCloudUrl + id + '.png';
			else response = ajax.apiObjectUrl('img', id); // download the image.
		}
		else if(tmplVars.id)
		{
	...
	widget._drawPreviewImageUpload = function($widgEl, tmplVars) {
		$widgEl.get(0).innerHTML = '<img class="preview-upload-image">';
		$imgEl = $widgEl.find('img');

        $imgEl.get(0).onerror = function() {
            if(this.src.indexOf('/data/img') === -1) {
                this.onerror = function() {};
                this.src = widget.getImageData(tmplVars.data, false);
            }
        };
        $imgEl.get(0).src = widget.getImageData(tmplVars.data, true);

		//$imgEl.attr('src', widget.getImageData(tmplVars.data, true));
	};
	...
	widget._drawImageUpload = function(widgEl, tmplVars)
	{
		...
		if(tmplVars.useCanvas) // When canvas is required, load the image and draw it in the canvas
		{
			...
			if (img.src.indexOf('/data/img') !== -1) {
				url = getImageData(false);
			}
			...
			img.onload = function(id) {
				if( window.uploadsCache && !window.uploadsCache[id] ) {
					if (!project.uploadCanvas || utils.isTaintedCanvas(project.uploadCanvas)) {
						project.uploadCanvas = document.createElement('canvas');
					}
	...
	{		
		...
		{
			if(window.g_preview) {
				widget._drawPreviewImageUpload(widgEl, tmplVars);
			} else {
	...
	widget.renderAbove = function(pageId, widgetId) {
		...
		for(var i=0; i<pageWidgets.length; i++) {
			if(above && storage.get(pageWidgets[i]).blur) {
				page.drawDetailed(pageId, 'init', {
                    restoreSelection: $('.pageWidget.ui-selected').attr('id')
                });
				return;
			}
	...
	{
        $('.widgetText').removeClass('editing');
	...
	widget.select = function (e, t, noContextMenu) // e can be null for drag events, loading. 
	{
	...
	widget._dragStop = function(e, ui, $this)
	{
        ...
        //when dragging a single, non-selected widget
        else {
            setTimeout(function() {
                selection.click();
            }, 50);
	...
	widget._libraryDrag = function (e, ui, $this)
	{
		...
		*/
		...
		if($this.parents("#uploadTabs").length == 1)
		{
 ...
 var ajax = 
 {
    ...
    generalRequest : function(info, type, callback, asPlainFormData)
    {
    ...
    // renamed userSettingsRequest to generalRequest and left userSettingsRequest for compatibility
    userSettingsRequest : function(info, type, callback, asPlainFormData)
    {
    	return ajax.generalRequest(info, type, callback, asPlainFormData);
    }
}
	...
	set: function(id, obj, changeUpdateTime)
	{
		...
		if(this.lastTabSynchronisationCheck + this.tabSynchronisationCheckInterval < new Date().getTime()) // no need to check every time?
		{
			...
			if(savedInstanceId && parseInt(savedInstanceId) != g_instanceId)// if saved instance id != current instance id, another tab has made a change in localstorage - refresh
			{
				...
				/*$('#blockEditing').fadeIn();
				$('#blockEditing button').click(function() {
	...
	sync : function (id, callback)
	{
		...
		project.loadIfUnloaded(id, function() {
			...
			if (toSync)
			{
				...
				//console.log(project.get('id'), id, project.lastVisiblePages[id], project.lastVisiblePages.pages, project.getPagesInViewport());
				if( project.get('id') !== id && project.lastVisiblePages && project.lastVisiblePages[id]) {
...
//            project.useLocalDb = false;
		...
		// hide page preview button
		$("#pagePreview", page.frame || document).css('opacity', 0);
...
//		project.startTime = new Date().getTime();
	...
	add : function (e, name, bypassLimit)
	{
		...
		{
			var name = '';
			var projectsCount = storage.get('projectsCount');
			var projectIds = account.get('projectIds');
			if (!projectsCount) {
				projectsCount = projectIds.length+1;
			} else {
				projectsCount += 1;
			}
			// this function will increment our newly created project name
			// if theres project named this way, to avoid dupe projects' names
			function checkName(currentName) {
				projectIds.forEach(function(projectId) {
					if (storage.get(projectId).name === currentName) {
						projectsCount += 1;
						currentName = checkName('Project '+projectsCount);
					}
				});
				return currentName;
			};
			storage.set('projectsCount', projectsCount);
			name = checkName('Project '+projectsCount);
		}
	...
	del : function(id, callback)
	{
		function removeProject()
		{
			...
			lib.handleUploadsRemovedFromPage(Object.keys(project.bundle(id).uploads), id, $('#project-'+id));
...
//			return false;
	...
	rename : function(e)
	{
		...
		fluid.main.trigger('projectNameChanged');
    ...
    getActivePage: function() {
    	return $('#canvasPages > .screen.active').first();
    },

    getActivePageId: function() {
    	return project.getActivePage().attr('id');
    }
};
...
var utils = 
{	
	reloadWithMessage : function(msgId)
	{
		function doReload()
		{
			storage._setItem("postloadMessage", msgId);
			location.reload();
		}

		if (fluid.main.deferReload)
		{
			fluid.main.on('deferReloadRelease', doReload);
		}
		else
		{
			doReload();
...
//		var canvLeft = parseFloat($("#canvas").css("left"));	// check for auto?
	...
	Vector: (function () {
		...
		//     frame : { width, height }
		// }
		// returns an object with width and height that fit within the frame
		getFitSize:function (sizes) {
			var referenceDimension,
				otherDimension,
				largerThanFrame = sizes.object.width > sizes.frame.width || sizes.object.height > sizes.frame.height,
				frameRatio = sizes.frame.width / sizes.frame.height,
				objectRatio = sizes.object.width / sizes.object.height;

			// If the object fits within the frame
			// and object expansion is not wanted
			if (!largerThanFrame && !sizes.expand) {
				// Return the object's original size
				return sizes.object;
			}
			...
			// If the frame is wider than the object
			if (frameRatio > objectRatio) {
				// Set the object height to the frame height
				// and change the width to maintain aspect ratio
			...
			newSize[referenceDimension] = sizes.frame[referenceDimension];
			newSize[otherDimension] = (sizes.frame[referenceDimension] / sizes.object[referenceDimension]) * sizes.object[otherDimension];

			return newSize;
		},

		isTaintedCanvas: function (context) {
			context = context instanceof HTMLCanvasElement
        ...
        if(!this.useOneCanvas) {
            ...
            if(font){
                ...
                if( style.indexOf('font-style') !== -1 ) {
                    fontStyle = style.substring(
                        style.indexOf('font-style')+11,
                        style.indexOf(';', style.indexOf('font-style'))
                    ).trim();
                }
                var fontVariant = 'normal';
                if( style.indexOf('font-variant') !== -1 ) {
                    fontVariant = style.substring(
                        style.indexOf('font-variant')+13,
                        style.indexOf(';', style.indexOf('font-variant'))
                    ).trim();
                }
                var fontWeight = 'normal';
                if( style.indexOf('font-weight') !== -1 ) {
                    fontWeight = style.substring(
                        style.indexOf('font-weight')+12,
                        style.indexOf(';', style.indexOf('font-weight'))
                    ).trim();
                }
                ...
                if (wdgObj.blur) {
                    if (renderer.renderBlur) {
						...
						if(widgetsCache && widgetsCache[wdgObj.id]) {
                            ...
                            if(!pageBox) {
                                var pageObj = storage.get(pageId);
                                pageBox = [0,0,pageObj.width, pageObj.height];
                            }
...
//            var imgData = this.canvases[pageId].get(0).toDataURL('image/jpeg', 0.5);
    ...
    this.onRenderFinish = function() {
        ...
        if (typeof window.callPhantom === 'function') {
            ...
            $('canvas.page-preview').each(function(){
                ...
                var pageObj = storage.get(pageId);
                if(!pageObj || !pageObj.widgets || !pageObj.widgets.length) {
                    return;
                }
                var data = $(this).get(0).toDataURL('image/jpeg', 0.5);
                //var data = $(this).get(0).toDataURL('image/png');
				if(pagesCache[pageId]) pagesCache[pageId].img = data;
    ...
    this.onImgLoad = function(pageId, src, evt) {
		...
		if( src.indexOf('i_') === 0 && window.uploadsCache && !window.uploadsCache[src] ) {
			...
			project.uploadCanvas.width = evt.target.naturalWidth;
			project.uploadCanvas.height = evt.target.naturalHeight;
			project.uploadCanvasCtx = project.uploadCanvas.getContext('2d');
...
//                    }
    ...
    this.foo = function(wdgObj, container) {
        ...
        if(!wdgObj.ca || wdgObj.ca[1] == 'l') {
            ...
            else box[0] = container.box[0] + container.box[2] - width - parseInt(left,10);
        }
    ...
    this.renderPage = function(pageObj, hideHotspots) {
...
//        console.log('pageObj: ', pageObj);
        ...
        //render dummy white background - needed as backgrounds with transparency causes some artifacts
        ...
        for(var prop in pageObj.widgets) {
            //dont render backgrounds for a second time && don't render hotspots if hideHotspots == true
            if((!pageObj.widgets[prop].st || pageObj.widgets[prop].st !== 'b') && !(hideHotspots && pageObj.widgets[prop].dm === 't')) {
...
widget.getBlurBox = function(widgetId, pageId) {
	var box, widgetBox, pageBox;

	if(pagesCache[pageId] && pagesCache[pageId].box) {
		pageBox = pagesCache[pageId].box;
	} else {
        var pageObj = storage.get(pageId);
        pageBox = [0,0,pageObj.width, pageObj.height];
...
//		}
	...
	if(widgetsCache[widgetId] && widgetsCache[widgetId].box) widgetBox = widgetsCache[widgetId].box;
...
widget.renderBlurLayer = function (widgetId, pageId, callback, onerror) {
	...
	blurBox = widget.getBlurBox(widgetId, pageId);
...
var share =
{
    ...
    init : function(projectId)
    {
        ...
        fluid.main.on('projectOpened,projectSharingOpened', function() {
            $('.shareLinkExportLocalFull, .shareLinkExportLocal').siblings('.msg').find('a').remove();
        });
    },
    ...
    actionParseSnapshot : function(respData, textStatus, jqXHR) // ajax response
    {
		...
		if(answer.a === 'PAGES') $msgBox = $('#previewShareContainer .shareLinkExportImg').
			add('#projectSettingsShareTop .shareLinkExportImg').next('.msg');
		else $msgBox = $('#previewShareContainer .shareLinkExportImgFull').
			add('#projectSettingsShareTop .shareLinkExportImgFull').next('.msg');
	...
	pollExport: function(id) {
		$.ajax({
			...
			success: function(result, textStatus, jqXHR) {
				...
				if(resultData.stype === 'PAGES') $msgBox = $('#previewShareContainer .shareLinkExportImg').
					add('#projectSettingsShareTop .shareLinkExportImg').next('.msg');
				else $msgBox = $('#previewShareContainer .shareLinkExportImgFull').
					add('#projectSettingsShareTop .shareLinkExportImgFull').next('.msg');
    ...
    {
        var zip, zipFile, base64strings = [],
            projectId = share.lastProjectId,
            projectObj = storage.get(projectId),
            projectName = projectObj.name,
            pos, pages, count = 0,
            folder, prepareExport, queueRasterizedPage, minMax, addFile, generateZip, linesOnTop = false; // hoisting
        ...
        var match = e.target.classList.toString()
                        ...
                        .match(/shareLinkDownload|everUsedExport|shareLinkPrint|shareLinkExportLocalFull|shareLinkExportLocal/);
    ...
    //         }
            ...
            else if (el.hasClass('shareLinkExportLocal'))
            {
                notification.add( "alert" , "Export in progress. Please stick with us, this can take up to a minute." );
                if (window.JSZip) {
                    zip = new JSZip();
                    folder = zip.folder(sanitizeFilename(projectName));
                    addFile = function(fileName, fileData) {
                        if (fileName && fileData) {
                            fileName = sanitizeFilename(fileName);
                            fileData = fileData.slice(fileData.indexOf(',')+1);
                            folder.file(fileName, fileData, {base64: true});
                        } else {
                            notification.add( "alert" , 'Baah! Something went wrong...' );
                            // console.log('wrong fileName or fileData');
                        }
                    };
                    generateZip = function() {
                        var blob = zip.generate({type:"blob"});
                        var link = document.createElement('a');
                        link.textContent = 'Download ('+(new Date().toLocaleString())+')';
                        link.href = window.URL.createObjectURL(blob);
                        link.download = sanitizeFilename(projectId)+'.zip';
                        $('.shareLinkExportLocal').siblings('.msg').find('a').remove();
                        $('.shareLinkExportLocal').siblings('.msg').append(link);
                        link.click();
                    };
                    pages = projectObj.pages;
                    setTimeout(function() {
                        pages.forEach(function(pageId) {
                            page.drawRasterised(pageId, function(id, base64) {
                                addFile(id+'.png', base64);
                                count += 1;

                                if (pages.length === count) {
                                    // last page was added - generate zip!
                                    generateZip();
                                }
                            }, true /*hide hotspots*/);
                        });
                    ...
                    notification.add( "alert" , 'Baah! Something went wrong...' );
                    // console.log('zip plugin not initialized!');
                }
            }
            else if (el.hasClass('shareLinkExportLocalFull'))
            {
                notification.add( "alert" , "Export in progress. Please stick with us, this can take up to a minute." );
                minMax = function(elements, max) {
                    var minMaxEl = elements[0];
                    var i;
                    if (max) {
                        for (i = 1; i < elements.length; i++) {
                            if (elements[i] > minMaxEl) {
                                minMaxEl = elements[i];
                            }
                        }
                    } else {
                ...
                pages = projectObj.pages;
                var pageElements = {};
                var xPoints = [], yPoints = [], xBorders = [], yBorders = [];
                var extremes = {
                    xMin: null,
                    yMin: null,
                    xMax: null,
                ...
                pages.forEach(function(pageId) {
                    var currentPage = storage.get(pageId);
                    var rightBorder = currentPage.x+currentPage.width;
                    var bottomBorder = currentPage.y+currentPage.height;
                    ...
                    xPoints.push(currentPage.x);
                    yPoints.push(currentPage.y);
                    xBorders.push(rightBorder);
                    yBorders.push(bottomBorder);
                });

                extremes.xMin = minMax(xPoints);
                extremes.xMax = minMax(xBorders, true);
                extremes.yMin = minMax(yPoints);
                extremes.yMax = minMax(yBorders, true);

                // at this point all pages are done rasterizing
                // accesible in pageElements object
                prepareExport = function(pageElements, linesOnTop) {
                    var svg = '', svg1 = '', downloadLink, blob, firstPage = pageElements[pages[0]];
                    // I'm assuming that every page is the same with and height here
                    // taking first page's one
                    var overallWidth = extremes.xMax - extremes.xMin + 2 * extremes.padding;
                    var overallHeight = extremes.yMax - extremes.yMin + 2 * extremes.padding;

                    var calculateLine = function(originWidget, originPage, destPage) {
                        ...
                        // center of widget
                        oTop = originPage.y + originWidget.top + originWidget.height/2;
                        oLeft = originPage.x + originWidget.left + originWidget.width/2;

                        // center of target page
                        dTop = (destPage.y + destPage.height / 2);
                        dLeft = (destPage.x + destPage.width / 2);
                        ...
                        if (Math.abs(diffQuotient) > destPage.height / destPage.width) {
                            if (dTop > oTop) {
                                endX = pageMidDiffX - destPage.height / (2 * diffQuotient);
                            } else {
                                endX = pageMidDiffX + destPage.height / (2 * diffQuotient);
                            }
                            ...
                            if (dLeft > oLeft) {
                                endX = pageMidDiffX - destPage.width / 2;
                            } else {
                                endX = pageMidDiffX + destPage.width / 2;
                            }
                        ...
                        var d = Math.sqrt(Math.pow((startX-endX), 2) + Math.pow((startY - endY), 2)); // distance
                        var r = 20 / d; // segment ratio

                        var x3 = r * startX + (1 - r) * endX; // find point that divides the segment
                        var y3 = r * startY + (1 - r) * endY; // into the ratio (1-r):r

                        return {
                            x1: startX - extremes.xMin + extremes.padding,
                            x2: x3 - extremes.xMin + extremes.padding,
                            y1: startY - extremes.yMin + extremes.padding,
                            y2: y3 - extremes.yMin + extremes.padding
                        };
                    };
                    var getLine = function(x1,y1,x2,y2) {
                        return '<line stroke-linecap="butt" x1="'+x1+'" y1="'+y1+'" x2="'+x2+'" y2="'+y2+'" marker-end="url(#triangle)" stroke="#3fa9f5" stroke-width="10"/>\n';
                    };
                    var links = projectObj.links;
                    var marker = '<marker xmlns="http://www.w3.org/2000/svg" id="triangle" viewBox="0 0 10 10" refX="3.5" refY="5" fill="#3fa9f5" markerUnits="strokeWidth" markerWidth="4" markerHeight="3" orient="auto"><path d="M 0 0 L 10 5 L 0 10 z"/></marker>\n';
                    svg += '<?xml version="1.0" encoding="utf-8"?>\n';
                    svg += '<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">\n';
                    svg += '<svg version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" width="'+overallWidth+'px" height="'+overallHeight+'px">\n';
                    svg += marker;

                    svg1 = svg;

                    var genratedLines = generateLines();
                    var generatedPages = generatePages();

                    function generatePages() {
                        var svgPart = '';
                        svgPart += '<g>\n';
                        pages.forEach(function(id) {
                            var currentPage = pageElements[id];
                            var relativeX = currentPage.x - extremes.xMin + extremes.padding;
                            var relativeY = currentPage.y - extremes.yMin + extremes.padding;
                            var width = currentPage.width;
                            var height = currentPage.height;
                            svgPart += '    <image xlink:href="'+currentPage.data+'" x="'+relativeX+'" y="'+relativeY+'" height="'+height+'px" width="'+width+'px"/>\n';
                            svgPart += '    <rect x="'+relativeX+'" y="'+relativeY+'" height="'+height+'px" width="'+width+'px" stroke="black" strokeWidth="2" fill="none"/>\n';
                        });
                        svgPart += '</g>\n';
                    ...
                    function generateLines() {
                        var svgPart = '';
                        // add links
                        if (links.linkOrigin && links.linkOrigin.length) {
                            svgPart += '<g>\n';
                            links.linkOrigin.forEach(function(origin, index) {
                                var destinationPage = storage.get(links.linkDest[index]);
                                var originWgt = widgetsCache[origin];
                                var originPage = storage.get(originWgt.pageId);

                                if (originPage && destinationPage) {
                                    var originWidget = {};
                                    originWidget.left = originWgt.box[0];
                                    originWidget.top = originWgt.box[1];
                                    originWidget.width = originWgt.box[2];
                                    originWidget.height = originWgt.box[3];

                                    calcLine = calculateLine(originWidget,originPage,destinationPage);
                                    svgPart += getLine(calcLine.x1,calcLine.y1,calcLine.x2,calcLine.y2);
                                }
                            });
                            svgPart += '</g>\n';
                        }
                ...
                generateZip = function() {
                    ...
                    link.href = window.URL.createObjectURL(blob);
                    link.download = sanitizeFilename(projectId)+'.zip';
                    $('.shareLinkExportLocalFull').siblings('.msg').find('a').remove();
                    $('.shareLinkExportLocalFull').siblings('.msg').append(link);
                    link.click();
                };

                queueRasterizedPage = function() {
                    var pageId = pages[count], blob1,blob2,svgs, downloadLink;

                    page.drawRasterised(pageId, function(id, base64) {
                        pageElements[id].data = base64;
                        count += 1;
                        ...
                        if (pages[count]) {
                            ...
                            svgs = prepareExport(pageElements);

                            addFile(projectName+' visible links.svg', svgs.linksOnTop);
                            addFile(projectName+'.svg', svgs.normal);
